<template>
  <div class='calendar'>
    ...
    <div  class='calendar-item' v-for='(item,index) in calendarInfo' :key='index'>
      ...
      <div class="calendar-info border-bottom"  @click='headerClose(result)' v-for='(result,index) in item.calendarInfo.result' :key='index'>
        ...
        <!-- 在组件中引入重复组件  :原组件绑定数据名字 = 新组件需要重新绑定的数据 -->
